...
def embed_watermark(image, watermark, threshold=80, k=12, z=2):
    block_size = 8
    h, w = image.shape
    # image -= 128   #                                            这里减了128结果就出大问题，不减就很正常 ,遗憾的是我感觉要减T-T
    ...
    return np.clip(watermarked_image, 0, 255).astype(np.uint8),key   # 将像素值标准化到0-255，超出这个范围的值被裁减（-20变成0，300变成255）
...
def extract_watermark(watermarked_image, block_size=8, threshold=80, k = 12, key=None):
    # Step 1: 将水印图像像素值减去128
    watermarked_image = watermarked_image.astype(np.float32) - 128   # 这里不知道为什么减不减128没有影响
...
if __name__ == "__main__":
    ...
    (p,q) = (15,50)    # 这里可以调整观测的像素点
    watermark = (watermark_image >= threshold1).astype(np.uint8)
    # print(f"watermark: {watermark[p,q]}")

    # integrer le tatouage
    watermarked_image,key = embed_watermark(cover_image, watermark)
    # 将左上角32*32的块遮盖（显示为黑色）
    watermarked_image[128:256, 128:256] = 0
    # 假设 watermarked_image 是 numpy.ndarray 类型
    cv2.imwrite("output/watermarked_image_attacked.png", watermarked_image)


    # 提取水印
    extracted_watermark = extract_watermark(watermarked_image,key=key)
    ...
    '''
    不同的位置列表: nombre : 28  [[14, 31], [15, 47], [18, 39], [20, 13], [21, 50], [25, 33], [26, 44], [33, 14], [33, 43], [34, 20], [34, 24],
